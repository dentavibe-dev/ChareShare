...
interface AddPackagePopupProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (packageData: Omit<PackageData, 'id'>) => void;
}

export const AddPackagePopup: React.FC<AddPackagePopupProps> = ({
  isOpen,
  onClose,
  onSave
}) => {
  ...
  const handleSave = async () => {
    ...
    setTimeout(() => {
      const newPackage = {
        ...
        title: serviceOptions.find(opt => opt.value === formData.service)?.label || 'Service',
        ...
        <div className="bg-white rounded-t-3xl px-6 py-8 mx-4 mb-4 shadow-2xl max-h-[80vh] overflow-y-auto">
          ...
          <div className="flex space-x-4 mt-8">
            ...
            >
              ...
              <span>{isSaving ? 'Adding...' : 'Add Package'}</span>
            </button>
          </div>
        </div>
      </div>
